...
export default function App() {
  ...
  const [outputMode, setOutputMode] = useState<'preview' | 'clean'>('clean');
  ...
  const totalN = useMemo(() => vis.reduce((s, c) => s + getTotalTags(c), 0), [vis]);
  ...
  const totalLimitsSum = useMemo(() => {
    ...
    for (const cat of enabledCats) {
      ...
      for (const sub of getVisibleSubcategories(cat)) {
    ...
    return sum;
  }, [enabledCats, limits, isSubEnabled]);
  ...
  const toggleCat = useCallback((cat: TagCategory) => {
    ...
    setDisabledSubs(p => {
      const next = { ...p };
      for (const s of subs) next[`${cat.id}:${s.id}`] = allEnabled;
      return next;
    });
  }, [isSubEnabled]);
  const enableAllCats = useCallback(() => {
    setDisabledSubs(p => {
      ...
      for (const cat of vis) {
        for (const s of getVisibleSubcategories(cat)) next[`${cat.id}:${s.id}`] = false;
      }
      return next;
    });
  }, [vis]);
  const disableAllCats = useCallback(() => {
    setDisabledSubs(p => {
      const next = { ...p };
      for (const cat of vis) {
        for (const s of getVisibleSubcategories(cat)) next[`${cat.id}:${s.id}`] = true;
      }
      return next;
    });
  }, [vis]);
  ...
  const generate = useCallback((batchN = 1) => {
    ...
    for (let b = 0; b < batchN; b++) {
      ...
      let remaining = tagCount - result.length;
      if (remaining <= 0) {
        newHist.push({ id: Date.now() + b, prompt: result.slice(0, tagCount).join(', ') });
        continue;
      }
      ...
      for (const cat of randomizedCats) {
        ...
        const subs = shuffle(getVisibleSubcategories(cat).filter(s => isSubEnabled(cat.id, s.id)));
        if (subs.length === 0) continue;
        if (catLimit > 0) {
          const tags = shuffle(subs.flatMap(s => s.tags.map(toTag)).filter(t => !blacklistSet.has(t.toLowerCase())));
          limitedPools.push({ catId: cat.id, tags, budget: catLimit });
        } else {
          for (const sub of subs) {
            const subLimit = limits[`${cat.id}:${sub.id}`] || 0;
            const tags = shuffle(sub.tags.map(toTag).filter(t => !blacklistSet.has(t.toLowerCase())));
  ...
  }, [enabledCats, limits, isSubEnabled, tagCount, whitelistTags, blacklistSet]);
    ...
    <div className="min-h-screen" style={{ background: '#0a0a0e' }}>
      ...
      <div className="max-w-[1480px] mx-auto px-5 sm:px-6 lg:px-8 xl:px-10 py-4 sm:py-5">
        <div className="flex flex-col lg:flex-row gap-5 xl:gap-6">
          <div className="flex-1 min-w-0 flex flex-col gap-3 lg:pr-1 xl:pr-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-2 gap-3">
              {vis.map(cat => {
                ...
                return (
                  <CatCard key={cat.id}
                    cat={cat} anySubOn={anySubOn} allSubOn={allSubOn}
                    limits={limits}
          ...
          <div className="lg:w-[440px] xl:w-[470px] flex flex-col gap-3 shrink-0 lg:pl-1 xl:pl-2">
                ...
                <div className="flex items-center gap-1.5">
                  {outputTags.length > 0 && (
                    <div className="flex rounded overflow-hidden" style={{ border: '1px solid rgba(255,255,255,.12)' }}>
                      <button onClick={() => setOutputMode('clean')} className={`px-2 py-0.5 text-[9px] font-medium ${outputMode === 'clean' ? 'bg-indigo-500/20 text-indigo-400' : 'text-white/20 hover:text-white/40'}`}>干净输出</button>
                      <button onClick={() => setOutputMode('preview')} className={`px-2 py-0.5 text-[9px] font-medium ${outputMode === 'preview' ? 'bg-indigo-500/20 text-indigo-400' : 'text-white/20 hover:text-white/40'}`} style={{ borderLeft: '1px solid rgba(255,255,255,.12)' }}>划掉预览</button>
                    </div>
                  )}
                  <div className="flex rounded overflow-hidden" style={{ border: '1px solid rgba(255,255,255,.12)' }}>
                    <button onClick={() => setDisplayMode('space')} className={`px-2 py-0.5 text-[9px] font-medium ${displayMode === 'space' ? 'bg-emerald-500/20 text-emerald-400' : 'text-white/20 hover:text-white/40'}`}>空格</button>
                    <button onClick={() => setDisplayMode('underscore')} className={`px-2 py-0.5 text-[9px] font-medium ${displayMode === 'underscore' ? 'bg-emerald-500/20 text-emerald-400' : 'text-white/20 hover:text-white/40'}`} style={{ borderLeft: '1px solid rgba(255,255,255,.12)' }}>_模式</button>
                  </div>
                  <button onClick={() => cleanedDisplayPrompt && copy(cleanedDisplayPrompt)} disabled={!cleanedDisplayPrompt} className={`text-[10px] font-semibold px-2 py-0.5 rounded ${copied ? 'text-emerald-400' : cleanedDisplayPrompt ? 'text-indigo-400 hover:text-indigo-300' : 'text-white/10'}`}>{copied ? '✓ Copied' : 'Copy'}</button>
                </div>
              </div>
              <div className="px-4 py-3 min-h-[56px] max-h-[190px] overflow-y-auto">
                {outputTags.length > 0 ? (
                  outputMode === 'clean' ? (
                    <code className="text-[11px] leading-[1.8] text-emerald-400/80 break-all select-all font-mono whitespace-pre-wrap block">{cleanedDisplayPrompt}</code>
                  ) : (
                    <code className="text-[11px] leading-[1.8] font-mono whitespace-pre-wrap block break-all">
                      {outputTags.map((tag, i) => {
                        ...
                        return (
                          <span key={`${tag}-${i}`}>
                            <span style={{ color: hidden ? 'rgba(255,255,255,.12)' : 'rgba(52,211,153,.8)', textDecoration: hidden ? 'line-through' : 'none' }}>{formatDisplayTag(tag)}</span>
            ...
            {outputTags.length > 0 && (
              <div className="rounded-xl overflow-hidden" style={{ background: '#101016', border: '1px solid rgba(255,255,255,.12)' }}>
                <div className="flex items-center justify-between px-4 h-9" style={{ borderBottom: '1px solid rgba(255,255,255,.10)' }}>
                  <span className="text-[10px] text-white/80 font-semibold">🏷️ 点击标签进行屏蔽 <span className="text-white/80 font-normal ml-1">({outputTags.length}个)</span></span>
                  <div className="flex items-center gap-1.5">
                    <button onClick={() => setHiddenOutputTags(new Set(outputTags))} className="text-[9px] text-white/80 hover:text-white px-1.5 py-0.5 rounded hover:bg-white/5">全选</button>
                    <button onClick={() => setHiddenOutputTags(new Set())} className="text-[9px] text-white/80 hover:text-white px-1.5 py-0.5 rounded hover:bg-white/5">清除</button>
                  </div>
                </div>
                <div className="px-4 py-3 max-h-[210px] overflow-y-auto">
                  <div className="flex flex-wrap gap-1">
                    {outputTags.map((tag, i) => {
                      ...
                      return (
                        <button key={`${tag}-${i}`} onClick={() => setHiddenOutputTags(prev => {
                          ...
                          style={{ background: isHidden ? 'rgba(244,63,94,.06)' : 'rgba(16,185,129,.07)', color: isHidden ? 'rgba(244,63,94,.5)' : 'rgba(52,211,153,.7)', textDecoration: isHidden ? 'line-through' : 'none', border: `1px solid ${isHidden ? 'rgba(244,63,94,.22)' : 'rgba(16,185,129,.22)'}` }}>
            ...
            <Panel title="✅ 白名单" subtitle="必定出现">
              <textarea value={whitelist} onChange={e => setWhitelist(e.target.value)} placeholder="逗号或换行分隔…" className="w-full h-16 bg-transparent text-[10px] text-white/50 placeholder:text-white/15 outline-none resize-none font-mono" />
            </Panel>
            <Panel title="🚫 黑名单" subtitle="永不出现">
              <textarea value={blacklist} onChange={e => setBlacklist(e.target.value)} placeholder="逗号或换行分隔…" className="w-full h-16 bg-transparent text-[10px] text-white/50 placeholder:text-white/15 outline-none resize-none font-mono" />
            </Panel>
            {hist.length > 0 && (
              <div className="rounded-xl overflow-hidden" style={{ background: '#101016', border: '1px solid rgba(255,255,255,.12)' }}>
                <button onClick={() => setShowHist(h => !h)} className="w-full flex items-center justify-between px-4 h-9 hover:bg-white/[.015]">
                ...
                {showHist && (
                  <div className="max-h-40 overflow-y-auto" style={{ borderTop: '1px solid rgba(255,255,255,.10)' }}>
                    {hist.map(h => (
                      <div key={h.id} onClick={() => copy(formatPromptByMode(h.prompt, displayMode))} className="px-4 py-2 cursor-pointer hover:bg-white/[.02]" style={{ borderBottom: '1px solid rgba(255,255,255,.08)' }}>
        ...
        <footer className="mt-6 pt-4 text-center" style={{ borderTop: '1px solid rgba(255,255,255,.10)' }}>
          <p className="text-[10px] text-white/10">Danbooru-style 随机提示词 · {totalN.toLocaleString()} tags</p>
        </footer>
      </div>
    </div>
  );
...
function CatCard({ cat, anySubOn, allSubOn, limits, pickedTags, displayMode, isOpen, subId,
  showLimits, tagCount, isSubEnabled,
  onToggleCat, onToggleSub, onOpen, onSubId, onLimitChange }: {
  cat: TagCategory; anySubOn: boolean; allSubOn: boolean;
    ...
    <div ref={ref} className="rounded-lg overflow-hidden" style={{
      ...
      <div className="flex items-center h-12 px-4 gap-3">
        ...
        <button onClick={onOpen} className="shrink-0 p-1.5 rounded hover:bg-white/[.08] transition-colors cursor-pointer" title="展开/收起标签">
          <svg className={`w-3.5 h-3.5 text-white/30 ${isOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
        </button>
        <button onClick={onToggleCat}
          ...
          {checkState !== 'none' && (checkState === 'all'
      ...
      {!isOpen && pickedTags.length > 0 && (
        <div className="px-4 pb-2.5 flex flex-wrap gap-1">
          {pickedTags.slice(0, 4).map(t => (
            <span key={t} className="rounded px-1.5 py-[2px] text-[9px] font-medium truncate max-w-[100px]" style={{ background: c + '14', color: c + 'cc', border: `1px solid ${c}38` }}>
              {formatTagByMode(t, displayMode)}
            </span>
          ))}
          {pickedTags.length > 4 && <span className="text-[9px] text-white/20 py-[2px]">+{pickedTags.length - 4}</span>}
        </div>
      )}
      {showLimits && !isOpen && (
        <div className="px-4 pb-3">
          <Slider label="分类限制" tooltip="此分类最多输出几个标签（0=不限）" value={catLimit} min={0} max={tagCount} onChange={v => onLimitChange(cat.id, v)} color={c} small />
        </div>
      )}
      {isOpen && (
        <div style={{ borderTop: '1px solid rgba(255,255,255,.10)' }}>
          ...
          {subs.length > 0 && (
            <div className="flex overflow-x-auto px-3 gap-2 py-2" style={{ borderTop: '1px solid rgba(255,255,255,.10)', borderBottom: '1px solid rgba(255,255,255,.12)', background: 'rgba(255,255,255,.05)' }}>
              {subs.map(s => {
                ...
                return (
                  <div key={s.id} className="shrink-0 flex items-center gap-2 rounded-md px-2.5 py-1.5" style={{ background: active ? 'rgba(255,255,255,.12)' : 'rgba(0,0,0,.25)', border: active ? `1px solid ${c}66` : '1px solid rgba(255,255,255,.10)' }}>
                    <button onClick={e => { e.stopPropagation(); onToggleSub(subKey); }} className="w-3.5 h-3.5 rounded-sm shrink-0 flex items-center justify-center cursor-pointer" title={subOn ? '点击屏蔽此子分类' : '点击启用此子分类'} style={{ background: subOn ? c + '99' : 'rgba(255,255,255,.06)', border: subOn ? 'none' : '1px solid rgba(255,255,255,.30)' }}>
                    ...
                    <button onClick={() => onSubId(s.id)} className="shrink-0 text-[10px] font-medium flex items-center gap-1.5 py-0.5 cursor-pointer" style={{ color: !subOn ? 'rgba(255,255,255,.20)' : active ? c : 'rgba(255,255,255,.65)', textDecoration: !subOn ? 'line-through' : 'none' }}>
                      {s.name}
          ...
          {showLimits && activeSub && isSubEnabled(cat.id, activeSub.id) && (
            <div className="px-4 pt-3">
              <Slider label={`${activeSub.name} 限制`} tooltip="此子分类最多输出几个标签（0=不限）" value={limits[`${cat.id}:${activeSub.id}`] || 0} min={0} max={tagCount} onChange={v => onLimitChange(`${cat.id}:${activeSub.id}`, v)} color={c} small />
            </div>
          )}
          {activeSub && (
            <div className="max-h-[180px] overflow-y-auto px-4 py-3" style={{ opacity: isSubEnabled(cat.id, activeSub.id) ? 1 : .25, background: 'rgba(0,0,0,.35)', borderTop: '1px solid rgba(255,255,255,.10)' }}>
              <div className="flex flex-wrap gap-2">
                {activeSub.tags.map(raw => {
                  const t = toTag(raw);
                  return (
                    <span key={raw} className="rounded-md px-2 py-1 text-[10px] leading-snug font-mono" style={{ background: 'rgba(255,255,255,.08)', color: 'rgba(255,255,255,.75)', border: '1px solid rgba(255,255,255,.15)' }}>
                      {formatTagByMode(t, displayMode)}
                    </span>
                  );
                })}
